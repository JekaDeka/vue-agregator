...
export default {
  ...
  mounted: function() {
    this.$store.dispatch('GET_DEVICE_MODELS')

  },
  computed: {
    List: function() {
      this.data = this.$store.getters.DEVICE_MODELS
      return this.data
    },
    ListData: function() {
      var list_result = [];
      var list = this.$store.getters.DEVICE_MODELS
  ...
  methods: {
    getUrl: function(item) {
      return item.brand + '/' + item.mark + '/' + item.id
    },
    setBefore: function() {
...
@import '@/assets/scss/device_models.scss';
</style>
