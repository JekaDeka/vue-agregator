...
export default {
  data: function() {
    return {
      seen: 0,
      title: "Услуга",
      error_msg: "Заполните сначала поле модель(или нет соответствия)",
      faults: [],
    }
  },
  methods: {
    showSelect: function() {
      ...
      if (this.$store.getters.BREAKAGE_ACTIONS.length != 0) {
        this.seen = 1;
        this.faults = this.$store.getters.BREAKAGE_ACTIONS
        return
      }
    ...
    select: function(obj) {
      this.title = obj.name
      this.seen = 0
      this.$store.dispatch('SELECT_BREAKAGE_ACTION', obj)
    },
  }
