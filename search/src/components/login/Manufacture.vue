  ...
  methods: {
    register: function() {
      if (this.login != "" && this.password != '') {
        this.$store.dispatch('GET_MANUFACTURE_BY_LOGIN_PASSWORD', [this.login, this.password])
        console.log('next')
        if (this.$store.getters.MANUFACTURE.length != 0) {
          var url = window.location.pathname
          var list = url.split('/')
          window.location.pathname = list[1] + '/' + 'register/manufacture/' + this.$store.getters.MANUFACTURE[0].id + '/'
        } else {
