...
<script>
import TheAdress from '@/components/master/Adress.vue'
import TheContactInformation from '@/components/master/ContactInformation'
import TheManufactureService from '@/components/master/ManufactureService'
import TheProposal from '@/components/master/Proposal'
import TheDelete from '@/components/master/Delete.vue'
export default {
  data: function() {
    el: '#changes'
    return {
  ...
  mounted: function() {
    ...
    if (isFinite(id)) {
      this.$store.dispatch('GET_MASTER', id)
      this.$store.dispatch('GET_TONWS_BY_COUNTRY_NAME', list[1])
      this.$store.dispatch('GET_NUMBERS_BY_MASTER', id)
    }
  },
  components: {
    TheAdress,
